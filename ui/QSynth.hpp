...
#define SYNTH_CONTROL(type, category, name)                                    \
  Q_PROPERTY(type category##_##name READ get_##category##_##name WRITE         \
                 set_##category##_##name NOTIFY controlChanged)                \
  type get_##category##_##name() { return m_synth[0].category.name; }          \
...
protected:
  ...
  Q_PROPERTY(
      int midiPort MEMBER m_midiPort WRITE setMidiPort NOTIFY midiPortChanged)
  int m_midiPort;
