...
QSynthDevice::QSynthDevice(Synth synth[2], QMutex &mutex, QObject *parent)
    : QIODevice(parent), m_synth(synth), m_mutex(mutex) {
...
qint64 QSynthDevice::writeData(const char *data, qint64 maxSize) { return -1; }

QSynth::QSynth(QObject *parent)
    : QSynthBase(parent), m_settings("./presets.ini", QSettings::IniFormat),
      m_midi(RtMidiIn::UNSPECIFIED,
             QCoreApplication::applicationName().toStdString()),
      m_midiPort(-1), m_device(m_synth, m_mutex, this) {
  initPreset();
        ...
        [](double delta, std::vector<unsigned char> *message, void *userData) {
          ...
          case 0x90: // note on
          {
            const float frequency =
                (440.f / 32.f) * std::pow(2.f, (data1 - 9.f) / 12.f);
            self.m_mutex.lock();
            self.m_synth[0].kbd.freq_target = self.m_synth[1].kbd.freq_target =
                frequency;
          ...
          case 0xB0: // control change
          {
            if (data1 == 1) {
              self.m_mutex.lock();
              self.m_synth[0].vcf.cutoff = self.m_synth[1].vcf.cutoff =
                  std::pow(data2 / 127.f, 2) * 16000;
              self.m_mutex.unlock();
          ...
          case 0xE0: // pitch bend
          {
            self.m_mutex.lock();
            const float value =
                ((int(data2) << 7) + int(data1)) / float(0b11111111111111) -
                0.5;
            const int sign = value >= 0 ? 1 : -1;
            self.m_synth[0].kbd.pitch_bend = self.m_synth[1].kbd.pitch_bend =
                sign * std::pow(sign * value, 2);
            self.m_mutex.unlock();
          } break;
          }
        },
...
QStringList QSynth::presetNames() { return m_settings.childGroups(); }
